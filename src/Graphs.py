...
# Grid Graph 8x4
def grid_graph_nxm(n,m,show=False):
...
def disjoint_union_c4(show=False):
...
def complete_bipartite_graph_k1616(show=False):
...
def hypercube_graph_q5(show=False):
...
# Wheel Graph W16
def wheel_graph_w16(show=False):
    G = nx.wheel_graph(16)
    if show:
        plt.figure(figsize=(8, 8))
        pos = nx.circular_layout(G)
...
def expander_graph_32(show=False):
...
def planar_connected_graph_16(show=False):
